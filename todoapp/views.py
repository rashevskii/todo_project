    ...
    if request.method == "GET":
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todoapp/signupuser.html', {'form': UserCreationForm(), 'error': 'That username '
                                                                                                        'is exist'})
        else:
            return render(request, 'todoapp/signupuser.html', {'form': UserCreationForm(), 'error': 'Passwords did not '
                                                                                                    'match'})
...
def loginuser(request):
    if request.method == "GET":
        ...
        if not user:
            return render(request, 'todoapp/loginuser.html', {'form': AuthenticationForm(), 'error': 'Passwords or '
                                                                                                     'login did not '
                                                                                                     'match'})
        else:
            login(request, user)
...
def createtodo(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todoapp/createtodo.html', {'form': TodoForm, 'error': 'Bad data'})
